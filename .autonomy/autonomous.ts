...
function context(state:NewsroomState,story:Story,p:AutonomousProgress):AutonomousContext{
  ...
  const sources=sourceItems(state,story).filter(s=>!s.demo&&s.type!=='email'&&s.type!=='social')
    .slice(-4).map(s=>({id:s.id,sourceName:s.sourceName,url:s.url,type:s.type,publishedAt:s.publishedAt,publishedAtKnown:s.publishedAtKnown,retrievedAt:s.retrievedAt,content:s.content.slice(0,4000)}));
...
export async function finishAutonomousStory(state:NewsroomState,story:Story,provider:ResearchProvider,deadline:number,save:()=>Promise<void>):Promise<void>{
  ...
  try{
    ...
    if(review.research.length){
      ...
      const fresh=progress(state,story);fresh.phase='research';fresh.feedback=review.research.map(r=>r.question);delete fresh.retrievalBasis;
    ...
    recordAssessedDraft(planned,story.id,{...bindings,storyId:undefined,...article,note:`Autonomous PE writer ${story.peAgentId}, independent checking PE ${p.reviewerId}; issue ${p.issueDate}. ${review.summary}`},true);
    // The input schema deliberately has no storyId. Remove accidental transport fields above.
