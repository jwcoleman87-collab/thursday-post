...
if(process.argv.includes('--run-live')) {
  ...
  assert.ok(result.state.runs.some(item=>touchedIds.has(item.storyId)&&item.agentType==='research'&&item.status==='completed'),'The PE handoff must be backed by a completed real researcher; HTTP 200 alone is insufficient');
  ...
  for(const stage of ['preflight','editorial'])assert.ok(operation.usage.some(item=>item.stage===stage&&item.inputTokens>0&&item.outputTokens>0),`Require actual ${stage} model usage`);
  if(freshRuns.some(item=>item.agentType==='research'))assert.ok(operation.usage.some(item=>item.stage==='research'&&item.inputTokens>0&&item.outputTokens>0),'Fresh research runs require actual research model usage');
