...
3. For daily collection, turn on **Monitoring** in Settings. The current schedule is 20:00 UTC: 6 am Sydney standard time, 7 am during daylight saving. With AI paused it only collects. Vercel runs this in the cloud; your computer does not need to stay on.
...
- [ ] **AI:** when you choose to activate it, finish Vercel AI Gateway verification, set its spending limits, change `NEWSROOM_AI_PAUSED=false` and redeploy. Run and review real research before relying on it. A successful source fetch does not prove live AI reporting.
