...
- [ ] **AI:** card verification and the US$5 monthly project limit are saved; OIDC preflight succeeds. The full hosted run failed exact-quote validation and then rate limits. AI is paused while daily source collection stays on. Fix the passage references and bounded retry pacing described in `CLAUDE_FINISH_PROMPT.md`, then unpause, redeploy and pass `node scripts/verify-live.mjs --run-live`. This requires actual research and PE model usage, evidence and a proposal; HTTP 200 or a source fetch alone cannot pass.
