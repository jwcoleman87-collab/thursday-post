...
const draftSchema = z.object({ id: identifier, headline: z.string(), byline: z.string(), peAgentId: peId, sentences: z.array(sentenceSchema), body: z.string(), hash: z.string().regex(/^[a-f0-9]{64}$/), createdAt: timestamp, limitations: z.array(z.string()), factReview: z.object({ actor: z.literal('James'), note: z.string(), reviewedAt: timestamp }).strict().optional(), reviewRevision: z.number().int().nonnegative().optional(), label: z.enum(['opinion', 'analysis', 'update', 'correction', 'right_of_reply']).optional(), deck: z.string().optional(), dateline: z.string().optional(), captions: z.array(z.object({ mediaId: identifier, text: z.string(), sourceIds: z.array(identifier) }).strict()).optional(), access: z.enum(['public', 'members']).optional() }).strict();
...
const gapSchema = z.object({ id: identifier, question: z.string(), agentId: researchId, status: z.enum(['open', 'resolved']), blocking: z.boolean(), resolution: z.string().optional(), claimIds: z.array(identifier), createdAt: timestamp }).strict();
...
const storySchema = z.object({ id: identifier, title: z.string(), summary: z.string(), mode, status: z.enum(['candidate', 'researching', 'drafting', 'waiting_approval', 'blocked', 'published', 'rejected', 'sent_back']), selectedReason: z.string(), researchAgentIds: z.array(researchId), peAgentId: peId, sourceItems: z.array(identifier), claims: z.array(claimSchema), findings: z.array(findingSchema), gaps: z.array(gapSchema), approvals: z.array(approvalSchema), compliance: z.array(complianceSchema), media: z.array(mediaSchema), wagering: z.boolean(), draft: draftSchema.optional(), draftHistory: z.array(draftSchema).optional(), proposedDraft: z.object({ headline: z.string(), sentences: z.array(sentenceSchema), researchRequests: z.array(z.object({ agentId: researchId, question: z.string() }).strict()).optional(), createdAt: timestamp, status: z.literal('requires_human_review') }).strict().optional(), correctionOf: z.string().optional(), correctionReason: z.string().optional(), access: z.enum(['public', 'members']).optional(), publishedEvidenceAlerts: z.array(z.object({ sourceIds: z.array(identifier), receivedAt: timestamp, status: z.enum(['open', 'reviewed']) }).strict()).optional(), editorialTone: z.enum(['A', 'B', 'N']).optional(), rightOfReply: z.object({ status: z.enum(['not_required', 'requested', 'received', 'declined', 'no_response']), note: z.string(), recipient: z.string().optional(), requestedAt: timestamp.optional(), deadline: timestamp.optional(), sourceIds: z.array(identifier), recordedAt: timestamp, actor: z.literal('James') }).strict().optional(), error: z.string().optional(), createdAt: timestamp, updatedAt: timestamp }).strict();
