  ...
  return <main className="operations">
    ...
    <section className="operations-section" id="edition-builder" aria-labelledby="builder-heading">
      ...
      <form onSubmit={saveDraft}>
        ...
        <p>The make-up desk arranges the pages itself: the strongest story leads, pictures and sections decide the rest, and nothing is added to fill space. An edition needs at least five substantial pages before it can be released.</p><button className="button secondary" type="button" disabled={disabled} onClick={() => setFields(current => ({ ...current, publicationIds: editionData.publications.map(publication => publication.id) }))}>Choose every approved article</button><fieldset><legend>Choose approved articles</legend>{editionData.publications.map(publication => <label className="operations-row" key={publication.id}><input type="checkbox" checked={fields.publicationIds.includes(publication.id)} onChange={event => setFields(current => ({ ...current, publicationIds: event.target.checked ? [...current.publicationIds, publication.id] : current.publicationIds.filter(id => id !== publication.id) }))} /><span>{publication.headline} · {publication.byline}</span></label>)}</fieldset>
    ...
    <section className="operations-section" id="edition-review" aria-labelledby="review-heading">
      ...
      {editionData.editions.length ? <><label>Choose an edition<select value={selectedEditionId} onChange={event => { setSelectedEditionId(event.target.value); setReviewConfirmed(''); setSendConfirmed(''); }}><option value="">Select an edition</option>{editionData.editions.map(edition => <option key={edition.id} value={edition.id}>No. {edition.number} · {edition.date} · {edition.title} · {edition.status === 'released' ? 'Released' : 'Draft'}</option>)}</select></label>{selectedEdition ? <>
        ...
        <h3>{selectedEdition.title}</h3><p>{selectedEdition.preview}</p><p>{selectedEdition.articles.length} articles. Last saved {dateLabel(selectedEdition.updatedAt)}.</p>{selectedEdition.pages ? (() => { const readiness = editionReadiness(selectedEdition.pages); return <p className={readiness.ready ? 'notice' : 'notice danger'}><strong>{selectedEdition.pages.length} composed pages</strong> ({selectedEdition.pages.map(page => page.section).join(' · ')}). {readiness.message} <Link href={`/editions/${selectedEdition.id}`}>Preview the composed pages ↗</Link></p>; })() : null}
    ...
    <section className="operations-section" id="published-stories" aria-labelledby="stories-heading">
      ...
      {newsroom.state.publications.filter(publication => publication.mode === 'live').length ? newsroom.state.publications.filter(publication => publication.mode === 'live').map(publication => <PublicationActions key={`${publication.id}-${publication.status}-${publication.access}-${publication.images?.length ?? 0}`} publication={publication} disabled={disabled} onAction={(body, message) => action(`publication-${publication.id}`, async () => { await api('/api/editorial', { method: 'POST', body }); return message; })} onPictures={(body, message) => action(`pictures-${publication.id}`, async () => { await api('/api/owner/pictures', { method: 'POST', body }); return message; })} />) : <p>There are no live published stories yet.</p>}
    </section>
...
function PublicationActions({ publication, disabled, onAction, onPictures }: { publication: Publication; disabled: boolean; onAction: (body: unknown, message: string) => Promise<void>; onPictures: (body: unknown, message: string) => Promise<void> }) {
